#!/usr/bin/env python3
"""Run waveguide/cavity_3D/waveguide3D full training (no max_steps override)."""
import os, re, subprocess

MODULUS_DIR = '/work/modulus-sym'
LOG_DIR = '/work/PaddleX_QA_test_new_science_max_step'
DIR_NAME = 'waveguide-cavity_3D-waveguide3D'
MODEL_PATH = 'waveguide/cavity_3D/waveguide3D'
...
def run(mode, env_vars):
    ...
    print(f'  cmd: {train_cmd[:160]}...', flush=True)
    with open(log_file, 'w') as f:
        subprocess.run(['bash', '-c', train_cmd], stdout=f, stderr=subprocess.STDOUT,
                      cwd=MODULUS_DIR, env=build_env(env_vars, mode), timeout=72000, check=False)
    steps = re.findall(r'\[step:\s*(\d+)\]', open(log_file, errors='ignore').read())
    print(f'  => max_step={max(int(s) for s in steps) if steps else 0}', flush=True)

print(f'=== {MODEL_PATH} full training (default max_steps=500000) ===', flush=True)
for mode, env_vars in [('dy', DY_ENV), ('cinn', CINN_ENV)]:
    try:
        run(mode, env_vars)
    except subprocess.TimeoutExpired:
        print(f'{mode} TIMEOUT (20h)', flush=True)
