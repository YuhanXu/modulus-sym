...
def run(cfg: ModulusConfig) -> None:
    ...
    flow_slv.saveable_models = flow_slv.get_saveable_models()
    flow_slv.global_optimizer_model = flow_slv.create_global_optimizer_model()
    from modulus.sym.trainer import Trainer
    Trainer._load_model(
        flow_slv.initialization_network_dir,
        flow_slv.network_dir,
        flow_slv.saveable_models,
        0,
        flow_slv.log,
        flow_slv.place,
    )
    ...
    for model in flow_slv.saveable_models:
        model.eval()
    ...
    if os.getenv("INFER_CHECK_ACCURACY", "0") == "1":
        ...
        for i in range(accuracy_iters):
            flow_slv.load_data()
            losses = flow_slv.compute_losses(i)
