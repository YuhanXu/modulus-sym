...
def run(cfg: ModulusConfig) -> None:
    ...
    thermal_slv.saveable_models = thermal_slv.get_saveable_models()
    thermal_slv.global_optimizer_model = thermal_slv.create_global_optimizer_model()
    from modulus.sym.trainer import Trainer
    Trainer._load_model(
        thermal_slv.initialization_network_dir,
        thermal_slv.network_dir,
        thermal_slv.saveable_models,
        0,
        thermal_slv.log,
        thermal_slv.place,
    )
    ...
    for model in thermal_slv.saveable_models:
        model.eval()

    print(f"[Inference] warmup_iters={warmup_iters}, bench_iters={bench_iters}")

    for i in range(warmup_iters):
        thermal_slv.load_data()
        _ = thermal_slv.compute_losses(i)
    ...
    if os.getenv("INFER_CHECK_ACCURACY", "0") == "1":
        ...
        for i in range(accuracy_iters):
            thermal_slv.load_data()
            losses = thermal_slv.compute_losses(i)
