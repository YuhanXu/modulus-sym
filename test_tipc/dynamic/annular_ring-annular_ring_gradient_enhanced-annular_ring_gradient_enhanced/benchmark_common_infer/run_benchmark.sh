...
# Test inference benchmark for a model.
...
function _set_params(){
    ...
    train_log_file=${run_log_path}/${model_repo}_${model_name}_${device_num}_infer_log
    speed_log_file=${speed_log_path}/${model_repo}_${model_name}_${device_num}_infer_speed
}
...
function _infer(){
    ...
    infer_cmd="pushd examples/annular_ring/annular_ring_gradient_enhanced; python annular_ring_gradient_enhanced_infer.py training.max_steps=0; popd"
    echo "pwd: $PWD infer_cmd: ${infer_cmd} log_file: ${train_log_file}"
    set -x
    timeout 15m bash -c "${infer_cmd}" > ${train_log_file} 2>&1
    if [ $? -ne 0 ];then
